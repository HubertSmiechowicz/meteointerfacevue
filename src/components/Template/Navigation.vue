<script setup lang="ts">
	...
	const changeDisplay = () => {
		if (menu.value)
			if (
				window.getComputedStyle(menu.value).getPropertyValue('display') ===
				'none'
			) {
...
<template>
	...
	<div
		...
		class="h-screen w-64 fixed top-0 -left-64 z-50 transition-transform duration-300 ease-in-out border-r rounded-r-3xl border-blue-500 dark:border-slate-50 md:flex md:flex-row md:h-full md:w-full md:static md:left-0 md:translate-x-0 md:border-none bg-slate-50 dark:bg-slate-800">
		<div
			...
			<NavItem content="Pięciodniowa" href="#/five-day" @click="toggleMenu" />
			<NavItem content="Powietrza" href="#/air-pollution" @click="toggleMenu" />
			<NavItem content="Kontakt" href="#/contact" @click="toggleMenu" />
		</div>
	</div>
</template>
